...
def sarima_forecast(df, coin_id, forecast_steps=30):
    ...
    try:
        ...
        ax.plot(sarimax_forecast.predicted_mean, label='Forecast', color='blue')
        ...
        ax.set_xlabel('Date')
        ax.set_ylabel('Price')
        ax.set_title(f'SARIMA Forecast for {coin_id}')
        ax.legend()

        # Save the figure
        fig_path = f'static/{coin_id}_sarima_forecast.png'
